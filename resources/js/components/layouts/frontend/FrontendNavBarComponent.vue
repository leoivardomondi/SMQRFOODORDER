        ...
        <div class="container flex flex-col lg:flex-row items-center justify-between">
            ...
            <div class="flex flex-col items-center justify-end sm:gap-3 w-full mt-3 lg:mt-0 lg:flex-row lg:w-fit">
                ...
                <div v-if="weather && weather.temp_c" 
                     class="hidden lg:flex items-center gap-1.5 px-3 h-8 rounded-3xl border border-gray-200 bg-[#FAF7EE] text-xs font-semibold text-heading shadow-2xs cursor-pointer transition-all hover:bg-amber-50"
                     :title="weather.is_raining ? 'Raining near branch - deliveries may take longer' : 'Current weather at branch'">
                    <span class="text-sm">{{ weather.icon || '⛅' }}</span>
                    <span>{{ weather.temp_c }}°C</span>
                    <span class="text-[11px] font-normal text-paragraph capitalize hidden xl:inline">{{ weather.condition_text }}</span>
                    <span v-if="weather.is_raining" class="flex h-2 w-2 relative ml-0.5" title="Live Rain Notice">
                ...
                <div v-else class="dropdown-group">
                    ...
                    <div
                        ...
                        <nav class="px-4">
                            ...
                            <router-link v-if="profile.role_id === enums.roleEnum.DELIVERY_BOY"
                                :to="{ name: 'frontend.rider.deliveries' }"
                                class="paper-link transition w-full flex items-center gap-3.5 py-2.5 border-b last:border-none border-[#EFF0F6]">
                                <i class="fa-solid fa-motorcycle lab-font-size-17"></i>
                                <span class="text-sm leading-6 capitalize">Manage Deliveries</span>
                            </router-link>

                            <router-link v-else :to="{ name: 'frontend.myOrder' }"
                                class="paper-link transition w-full flex items-center gap-3.5 py-2.5 border-b last:border-none border-[#EFF0F6]">
