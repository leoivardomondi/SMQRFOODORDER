<template>
    <nav
        ...
        <router-link :class="isDeliveryBoy ? (currentRoute.startsWith('/rider/deliveries') ? 'text-primary' : '') : (checkIsPathAndRoutePathSame('/my-orders') ? 'text-primary' : '')"
            ...
            :to="{ name: isDeliveryBoy ? 'frontend.rider.deliveries' : 'frontend.myOrder' }">
            <i class="fa-solid fa-receipt text-base leading-none"></i>
            <span class="text-xs capitalize">{{ $t('label.orders') }}</span>
        </router-link>
...
export default {
    ...
    computed: {
        ...
        isDeliveryBoy: function () {
            return this.$store.getters.authInfo?.role_id === 3;
        },
        categories: function () {
