<template>
    ...
    <div id="address-modal" class="modal address-modal ff-modal">
        <div class="modal-dialog address-form-dialog">
            <div class="modal-header border-none pb-0">
                <h3 class="capitalize font-medium">{{ $t('label.address') }}</h3>
                <button class="address-modal-close modal-close fa-regular lab lab-close-circle-line lab-font-size-22 lab-font-bold"
                        @click="reset"></button>
            </div>
                ...
                <form @submit.prevent="save">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea v-model="props.form.apartment"
                                  class="address-form-control h-12 w-full rounded-lg border py-1.5 px-2 placeholder:text-[10px] placeholder:text-[#6E7191] border-[#D9DBE9]"></textarea>
                    </div>
                    <div class="mb-6">
                        ...
                        <nav class="flex flex-wrap gap-3 active-group">
                            <button @click="changeSwitchLabel(labelEnum.HOME)"
                                    ...
                                    class="address-label-option flex items-center gap-2 rounded-lg p-4 border">
                        ...
                        <div v-if="props.status" :class="!props.status ? 'h-0' : ''" class="overflow-hidden transition">
                            <input type="text" :placeholder="$t('label.type_label_name')" v-model="props.form.label"
                                   ...
                                   class="address-form-control h-10 w-full rounded-lg border mt-5 py-1.5 px-4 placeholder:text-xs border-[#D9DBE9]">
                            <small class="db-field-alert" v-if="errors.label">{{ errors.label[0] }}</small>
                        </div>
                    </div>
                    <button type="submit"
                            class="address-confirm-button rounded-3xl text-base py-3 px-3 font-medium w-full text-white bg-primary">
