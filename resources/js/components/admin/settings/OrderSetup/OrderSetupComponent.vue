            ...
            <form @submit.prevent="save">
                ...
                <fieldset class="p-4 mb-6 border border-[#DBDEE0]">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-col-12 sm:form-col-6">
                            <label for="order_setup_max_delivery_kilometer" class="db-field-title">
                                Max Delivery Radius (Kilometers)
                            </label>
                            <input v-on:keypress="floatNumber($event)" v-model="form.order_setup_max_delivery_kilometer"
                                v-bind:class="errors.order_setup_max_delivery_kilometer ? 'invalid' : ''" type="text"
                                id="order_setup_max_delivery_kilometer" class="db-field-control" placeholder="0 for unlimited distance" />
                            <small class="db-field-alert" v-if="errors.order_setup_max_delivery_kilometer">{{
                                errors.order_setup_max_delivery_kilometer[0]
                            }}</small>
                        </div>
                    </div>
                </fieldset>
                ...
                <fieldset class="p-4 mb-6 border border-[#DBDEE0]">
                    <legend class="py-1.5 px-4 text-base font-semibold capitalize border border-[#DBDEE0] text-primary">
                        Trust Score & Pay on Delivery Policy
                    </legend>
                    <div class="form-row">
                        <div class="form-col-12 sm:form-col-6">
                            <label class="db-field-title required">Restrict Pay on Delivery by Trust Score</label>
                            <div class="db-field-radio-group">
                                <div class="db-field-radio">
                                    <div class="custom-radio">
                                        <input :value="enums.activityEnum.ENABLE" v-model="form.order_setup_trust_score_enable"
                                            id="trust-score-enable" type="radio" class="custom-radio-field" />
                                        <span class="custom-radio-span"></span>
                                    </div>
                                    <label for="trust-score-enable" class="db-field-label">{{ $t("label.enable") }}</label>
                                </div>
                                <div class="db-field-radio">
                                    <div class="custom-radio">
                                        <input :value="enums.activityEnum.DISABLE" v-model="form.order_setup_trust_score_enable"
                                            type="radio" id="trust-score-disable" class="custom-radio-field" />
                                        <span class="custom-radio-span"></span>
                                    </div>
                                    <label for="trust-score-disable" class="db-field-label">{{ $t("label.disable") }}</label>
                                </div>
                            </div>
                            <small class="db-field-alert" v-if="errors.order_setup_trust_score_enable">{{
                                errors.order_setup_trust_score_enable[0]
                            }}</small>
                        </div>

                        <div class="form-col-12 sm:form-col-6">
                            <label for="order_setup_trust_score_min_orders" class="db-field-title required">
                                Minimum Successful Orders Required
                            </label>
                            <input v-on:keypress="numberOnly($event)" v-model="form.order_setup_trust_score_min_orders"
                                v-bind:class="errors.order_setup_trust_score_min_orders ? 'invalid' : ''" type="text"
                                id="order_setup_trust_score_min_orders" class="db-field-control" />
                            <small class="db-field-alert" v-if="errors.order_setup_trust_score_min_orders">{{
                                errors.order_setup_trust_score_min_orders[0]
                            }}</small>
                        </div>
                    </div>
                </fieldset>
    ...
    data() {
        return {
            ...
            form: {
                ...
                order_setup_max_delivery_kilometer: null,
                order_setup_trust_score_enable: null,
                order_setup_trust_score_min_orders: null,
            },
            enums: {
        ...
        try {
            ...
            this.$store.dispatch('orderSetup/lists').then(res => {
                this.form = {
                    ...
                    order_setup_max_delivery_kilometer: res.data.data.order_setup_max_delivery_kilometer || "0",
                    order_setup_trust_score_enable: res.data.data.order_setup_trust_score_enable,
                    order_setup_trust_score_min_orders: res.data.data.order_setup_trust_score_min_orders,
                }
