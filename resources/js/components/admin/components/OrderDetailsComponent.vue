                ...
                <div v-if="parseInt(order.status) !== parseInt(enums.orderStatusEnum.REJECTED) && parseInt(order.status) !== parseInt(enums.orderStatusEnum.CANCELED)"
                    ...
                    <ul class="flex flex-col gap-2">
                        <li class="flex flex-wrap items-center gap-2">
                            ...
                            <span v-if="order.transaction" class="capitalize text-sm leading-6 text-heading font-semibold">
                                {{ order.transaction.payment_method }}
                                <span v-if="order.transaction.provider_name">({{ order.transaction.provider_name }})</span>
                                <span v-if="order.transaction.transaction_no"> - {{ order.transaction.transaction_no }}</span>
                            </span>
                            <span v-else class="capitalize text-sm leading-6 text-heading font-semibold">
                                {{ enums.paymentTypeEnumArray[order.payment_method] }}
                            </span>
                        </li>
                        <li v-if="order.transaction?.provider_receipt && order.transaction.provider_receipt !== order.transaction.transaction_no" class="flex items-center gap-2">
                            <span class="text-sm leading-6">Provider receipt:</span>
                            <strong class="text-sm leading-6 text-heading">{{ order.transaction.provider_receipt }}</strong>
                        </li>
                        <li v-if="order.transaction?.payer_phone" class="flex flex-wrap items-center gap-2">
                            <span class="text-sm leading-6">Paying contact:</span>
                            <strong class="text-sm leading-6 text-heading">{{ order.transaction.payer_phone }}</strong>
                        </li>
