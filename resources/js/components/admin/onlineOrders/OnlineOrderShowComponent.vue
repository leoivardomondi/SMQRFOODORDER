                ...
                <div>
                    ...
                    <ul class="flex flex-col gap-2">
                        ...
                        <li class="text-xs">
                            ...
                            <span class="text-heading font-semibold capitalize" v-if="order.transaction">
                                {{ order.transaction.payment_method }}
                                <span v-if="order.transaction.provider_name">({{ order.transaction.provider_name }})</span>
                            </span>
                            <span v-else class="text-heading font-semibold capitalize">
                                {{ enums.paymentTypeEnumArray[order.payment_method] }}
                            </span>
                        </li>
                        <li v-if="order.transaction" class="rounded-lg border border-[#D8C37A] bg-[#FFF9E8] p-3 text-xs">
                            <p class="mb-1.5 font-semibold text-heading">Payment Information</p>
                            <p class="mb-1">
                                Paid by:
                                <strong class="text-heading">{{ (orderUser && orderUser.name) ? orderUser.name : 'Guest' }}</strong>
                            </p>
                            <p class="mb-1" v-if="order.transaction.transaction_no">
                                Transaction / Receipt:
                                <strong class="text-heading">{{ order.transaction.transaction_no }}</strong>
                            </p>
                            <p class="mb-1" v-if="order.transaction.payer_phone">
                                Paying contact:
                                <strong class="text-heading">{{ order.transaction.payer_phone }}</strong>
                            </p>
                            <p class="mb-1">
                                Payment date:
                                <strong class="text-heading">{{ order.transaction.payment_date }}</strong>
                            </p>
                            <p>
                                Payment time:
                                <strong class="text-heading">{{ order.transaction.payment_time }}</strong>
                            </p>
                        </li>
                        <li v-if="order.transaction?.provider_receipt" class="text-xs">
                            M-PESA receipt:
                            <strong class="text-heading">{{ order.transaction.provider_receipt }}</strong>
                        </li>
                        <li v-if="order.transaction?.payer_phone" class="text-xs">
                            Paying contact:
                            <strong class="text-heading">{{ order.transaction.payer_phone }}</strong>
                            <span v-if="order.transaction.payer_phone_last4" class="text-paragraph">
                                (Paystack: {{ order.transaction.payer_phone_last4 }})
                            </span>
                        </li>
                        <li class="text-xs">
                            {{ $t('label.order_type') }}:
                    ...
                    <div class="db-card-body">
                        ...
                        <ul class="flex flex-col gap-3 py-4 border-[#EFF0F6]"
                            ...
                            <li class="flex items-center gap-2.5" v-if="orderUser && orderUser.phone">
                                ...
                                <span class="text-xs" dir="ltr">{{ internationalPhone(orderUser.country_code, orderUser.phone) }}</span>
                            </li>
                        </ul>
                        <div v-if="order.order_type === enums.orderTypeEnum.DELIVERY && orderAddress && orderAddress.address" class="flex items-start gap-3">
                            ...
                            <OnlineOrderMapComponent v-if="orderAddress.latitude && orderAddress.longitude" :orderAddress="orderAddress" />
                        </div>
                    </div>
                </div>
            </div>
...
export default {
    ...
    methods: {
        internationalPhone: function (countryCode, phone) {
            return appService.internationalPhone(countryCode, phone);
        },
