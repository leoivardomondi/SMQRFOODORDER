...
<body>
    <div class="py-14 px-4 w-full max-w-3xl mx-auto">
        ...
        <form id="paymentForm" method="POST" action="{{ URL::temporarySignedRoute('payment.store', now()->addMinutes(30), ['order' => $order]) }}">
            @csrf
            <fieldset class="payment-fieldset">
                ...
                @else
                    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mb-5 rounded relative"
                        role="alert">
                        <span class="block sm:inline">{{ __('all.message.payment_gateway_disable') }}</span>
                    </div>
                @endif
            </fieldset>
