...
<link rel="apple-touch-icon" sizes="152x152" href="{{ data_get($config['icons'], '152x152.src', data_get(end($config['icons']), 'src')) }}">
<link rel="apple-touch-icon" sizes="180x180" href="{{ data_get(end($config['icons']), 'src') }}">
<!-- Precomposed is retained for older iOS versions that do not fully process modern manifests. -->
<link rel="apple-touch-icon-precomposed" href="{{ data_get(end($config['icons']), 'src') }}">
