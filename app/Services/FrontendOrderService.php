    ...
    public function myOrderStore(OrderRequest $request): object
    {
        try {
            DB::transaction(function () use ($request) {
                if ($request->filled('customer_email') && Auth::user()->email !== $request->customer_email) {
                    Auth::user()->update(['email' => $request->customer_email]);
                }

                $paymentMethod = $request->payment_method;
                if ($paymentMethod === 'cash' || $paymentMethod === 'cash-on-delivery') {
                    $paymentMethod = 1;
                ...
                if ($paymentMethod == 1) {
                    $trustMetrics = app(\App\Services\TrustScoreService::class)->getUserMetrics(Auth::user());
                    if (!$trustMetrics['can_pay_on_delivery']) {
                        $minOrders = $trustMetrics['min_required_orders'];
                        throw new Exception("Pay on Delivery is restricted based on your Trust Score. You need at least {$minOrders} successful order(s) to pay on delivery. Please pay online.", 422);
                    }
                }
            ...
            if ($frontendOrder->user_id == Auth::user()->id) {
                if ($request->status == OrderStatus::CANCELED) {
                    if ($frontendOrder->payment_status == PaymentStatus::PAID) {
                        throw new Exception(trans('all.message.paid_order_cannot_cancel'), 422);
                    }
