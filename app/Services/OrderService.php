...
class OrderService
{
    ...
    public function list(PaginateRequest $request)
    {
        try {
            ...
            return Order::with('transaction', 'orderItems.orderItem', 'branch', 'user')->where(function ($query) use ($requests) {
    ...
    public function deliveryBoyOrder(PaginateRequest $request)
    {
        try {
            ...
            if ($request->get('available') == 1) {
                return Order::where('order_type', OrderType::DELIVERY)
                    ->where(function($q) {
                        $q->whereNull('delivery_boy_id')->orWhere('delivery_boy_id', 0);
                    })
    ...
    public function deliveryBoyOrderDetails(Order $order): Order|array
    {
        try {
            ...
            $isOpenDelivery = $order->order_type === OrderType::DELIVERY 
                && (!$order->delivery_boy_id || $order->delivery_boy_id == 0)
                && $order->status === OrderStatus::ACCEPT;

            if ($isAssigned || $isOpenDelivery) {
                return $order;
            } else {
                return [];
            }
    ...
    {
        try {
            abort_unless($order->delivery_boy_id === Auth::id(), 403);

            $targetStatus = (int) $request->status;
            $canStartDelivery = in_array($order->status, [
                OrderStatus::ACCEPT,
                OrderStatus::PREPARING,
                OrderStatus::PREPARED,
            ], true) && $targetStatus === OrderStatus::OUT_FOR_DELIVERY;
            $canCompleteDelivery = $order->status === OrderStatus::OUT_FOR_DELIVERY
                && $targetStatus === OrderStatus::DELIVERED;

            abort_unless($canStartDelivery || $canCompleteDelivery, 422, 'This delivery status change is not allowed.');
